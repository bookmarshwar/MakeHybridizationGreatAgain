...
import static com.folk.MakeHybridizationGreatAgain.util.Utils.generetorTF;
...
import static ic2.core.item.ItemCropSeed.getGainFromStack;
import static ic2.core.item.ItemCropSeed.getGrowthFromStack;
import static ic2.core.item.ItemCropSeed.getResistanceFromStack;
...
public class AutomatedBreedingFacility extends BaseMachine<AutomatedBreedingFacility> implements IAddUIWidgets {
    ...
    private List<ItemStack> getNewSpeedAndTwo(ItemStack stack1, ItemStack stack2) {
        ...
        if (!k) {
            newseed = tec.generateSeeds(cropcard1, gr, ga, re, (byte) 4);
            setOverclockFromStack(newseed, getOverclockFromStack(stack1));

            seeds.add(newseed);
            newseed = tec.generateSeeds(cropcard2, gr, ga, re, (byte) 4);
    ...
    private ItemStack getNewSpeedAndOne(ItemStack stack) {
        ...
        ItemStack newseed = null;
        if (!k) {
            newseed = tec.generateSeeds(cropcard, gr, ga, re, (byte) 4);
    ...
    protected IAlignmentLimits getInitialAlignmentLimits() {
        return (direction, rotation,
            flip) -> (direction == ForgeDirection.NORTH || direction == ForgeDirection.SOUTH
                || direction == ForgeDirection.EAST
                || direction == ForgeDirection.WEST) && rotation == Rotation.NORMAL && flip == Flip.NONE;
    }
    ...
    public CheckRecipeResult checkProcessing() {
        ...
        switch (foundSeeds.size()) {
            case 1 -> {
                ...
                ItemStack stackToConsume = stack.copy();
            ...
            case 2 -> {
                ...
                List<ItemStack> newSeeds = getNewSpeedAndTwo(
                    foundSeeds.get(0)
                        .copy(),
                    foundSeeds.get(1)
                        .copy());

                mOutputItems = new ItemStack[] { newSeeds.get(0), newSeeds.get(1) };

                mMaxProgresstime = 100;
                mEfficiencyIncrease = 10000;
                this.count++;
                ItemStack stackToConsume1 = foundSeeds.get(0)
                    .copy();
                stackToConsume1.stackSize = 1;
                ItemStack stackToConsume2 = foundSeeds.get(1)
                    .copy();
    ...
    protected final String[][] STRUCTURE = new String[][] { { "BBBBB", "BBBBB", "BBBBB", "BBBBB", "BBBBB" },
        { "AAAAA", "A   A", "A   A", "A   A", "AAAAA" }, { "AAAAA", "A   A", "A   A", "A   A", "AAAAA" },
    ...
    public ModularWindow createMyWindow(EntityPlayer player) {
        ...
        ListButton displayModeButton = (ListButton) new ListButton()
            .setForEnum_unsafe(CrossingMode.class, CrossingMode.Re)
            ...
            .setTextureGetter(state -> { return new Text(CrossingMode.values()[state].getDisplayName()); })
            // 设置尺寸
            .setSize(80, 20)
            .setPos(4, 20);
    ...
    public void onRenderWorldLast(TileEntity te, RenderWorldLastEvent event) {
        ...
        if (gtTe.isActive())
            this.renderItemAt(targetX, targetY, targetZ, itemToRender, renderX, renderY, renderZ, angle, 2f);
