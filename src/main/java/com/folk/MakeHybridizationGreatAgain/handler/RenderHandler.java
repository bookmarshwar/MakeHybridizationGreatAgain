...
//
//        System.out.println(String.format("[RenderEventHandler] Searching for TileEntities in chunk [%d, %d]", getChunkX(cache), getChunkZ(cache)));
//
...
//            if (isTargetMyMachine(te)) {
//                System.out.println(String.format("[RenderEventHandler] -> Found a target machine at [%d, %d, %d]!", te.xCoord, te.yCoord, te.zCoord));
//                // 计算头部坐标
//                int headX = te.xCoord;
...
//        System.out.println(String.format("[RenderHijacker] DIAGNOSIS: Attempted to set block to air in world at [%d, %d, %d]. Success: %s", x, y, z, success));
//    }
...
//
//                    System.out.println("[RenderHijacker] SUCCESS: Forced chunk at [" + rendererPosX + ", " + rendererPosY + ", " + rendererPosZ + "] to rerender.");
//                    break; // 找到了就退出
//                }
//            }
//        } catch (Exception e) {
...
//            // 如果还失败，那真的就是天意了
//            throw new RuntimeException("RenderHijacker failed to find reflection fields. All names are confirmed from your source code.", e);
//        }
//    }
//
//
...
//            System.out.println(String.format("[RenderHijacker] Attempting to change block at [%d, %d, %d]. Original ID: %d", x, y, z, originalID));
//
//            // 5. 执行替换！
